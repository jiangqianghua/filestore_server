...
func OnUserFileUploadFinished(username string, filehash string, filename string, filesize int64) bool {
	...
	_, err1 := stmt.Exec(username, filehash, filename, filesize, time.Now())
	if err1 != nil {
		return false
	}
	return true
...
func QueryUserFileMeta(username string, limit int) ([]UserFile, error) {
	stmt, err := mysql.DBConn().Prepare("select file_sha1, file_name, file_size, upload_at, last_update from tbl_user_file " +
